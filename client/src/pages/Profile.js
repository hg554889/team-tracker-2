  ...
  async function save(e){
    ...
    try{
      ...
      if (Object.keys(payload).length === 0 && hasClub) {
        window.dispatchEvent(new CustomEvent('toast',{ detail:{ type:'info', msg:'변경할 내용이 없습니다.'} }));
        setSaving(false);
        return;
      }
      ...
      <div className="profile-content">
        <form onSubmit={save} className="profile-form">
          <div className="form-section">
            ...
            <div className="form-group">
              <label>이메일</label>
              <input className="form-input disabled" value={user?.email || ''} disabled />
            </div>
            ...
            <div className="form-group">
              <label>학번</label>
              <input className="form-input disabled" value={user?.studentId || ''} disabled />
            </div>
            
            <div className="form-group">
              <label>현재 권한</label>
              <input className="form-input disabled" value={getRoleDisplay(user?.role)} disabled />
            </div>

            {!hasClub ? (
              <div className="form-group">
                <label>동아리 선택 (최초 1회)</label>
                <select className="form-input" value={clubId} onChange={e=>setClubId(e.target.value)} required>
              ...
              <div className="form-group">
                <label>동아리</label>
                <input className="form-input disabled" value={clubName} disabled />
                <div className="form-hint">
                  ⚠️ 동아리는 최초 1회만 선택 가능하며 프로필에서 변경할 수 없습니다.
                </div>
              </div>
            )}
        ...
        <div className="profile-actions">
          ...
          <div className="action-buttons">
            <button className="action-btn security" onClick={()=> setPwOpen(true)}>
              <span className="action-icon">🔒</span>
              <div className="action-content">
                <span className="action-title">비밀번호 변경</span>
                <span className="action-desc">계정 보안을 위해 정기적으로 변경하세요</span>
              </div>
            </button>

            <button className="action-btn support" onClick={() => setInquiryOpen(true)}>
              <span className="action-icon">💬</span>
              <div className="action-content">
                <span className="action-title">문의하기</span>
                <span className="action-desc">궁금한 점이나 문제가 있으면 언제든 문의하세요</span>
              </div>
            </button>

            {availableRoles.length > 0 && !hasPendingRequest && (
              <button className="action-btn role" onClick={() => setRoleRequestOpen(true)}>
                <span className="action-icon">⬆️</span>
                <div className="action-content">
                  <span className="action-title">권한 요청</span>
                  <span className="action-desc">더 높은 권한이 필요하시면 요청하세요</span>
                </div>
              </button>
            )}
          </div>
          ...
          <div className="modal-overlay" onClick={() => setPwOpen(false)}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-actions">
                <button className="btn-secondary" onClick={()=> setPwOpen(false)} disabled={pwLoading}>
                  취소
                </button>
        ...
        {/* Role Request Modal */}
        {roleRequestOpen && (
          <div className="modal-overlay" onClick={() => setRoleRequestOpen(false)}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                <h3>⬆️ 권한 요청</h3>
                <button className="modal-close" onClick={() => setRoleRequestOpen(false)}>×</button>
              </div>
              <div className="modal-body">
                <div className="form-group">
                  <label>요청할 권한</label>
                  <select className="form-input" value={requestedRole} onChange={e => setRequestedRole(e.target.value)} required>
              ...
              <div className="modal-actions">
                <button className="btn-secondary" onClick={() => setRoleRequestOpen(false)} disabled={roleRequestLoading}>
                  취소
                </button>
        ...
        {inquiryOpen && (
          <div className="modal-overlay" onClick={() => setInquiryOpen(false)}>
            <div className="modal-content large" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                <h3>💬 문의하기</h3>
                <button className="modal-close" onClick={() => setInquiryOpen(false)}>×</button>
              </div>
              <div className="modal-body">
                <div className="form-group">
                  <label>제목</label>
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    <label>우선순위</label>
                    <select className="form-input" value={inquiryPriority} onChange={e => setInquiryPriority(e.target.value)}>
              ...
              <div className="modal-actions">
                <button className="btn-secondary" onClick={() => setInquiryOpen(false)} disabled={inquiryLoading}>
                  취소
                </button>
                ...
                <div key={request._id} className="history-item">
                  <div className="history-header">
                    <div className="history-title">
                      <strong>{getRoleDisplay(request.currentRole)} → {getRoleDisplay(request.requestedRole)}</strong>
            ...
            <div className="history-list">
              {myInquiries.map(inquiry => (
                <div key={inquiry._id} className="history-item inquiry">
                  ...
                  <div className="history-content">
                    {inquiry.content.length > 150 ? inquiry.content.substring(0, 150) + '...' : inquiry.content}
                  </div>
                  {inquiry.response && (
