...
export default function InquiryManagement() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'in_progress': return { bg: 'var(--primary-light)', color: 'var(--primary)' };
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      ...
      case 'normal': return { bg: '#f0fdf4', color: '#16a34a' };
      case 'low': return { bg: '#f8fafc', color: '#64748b' };
      default: return { bg: '#f3f4f6', color: '#6b7280' };
    }
  };
        ...
        <div className="card" style={{ marginTop: '20px' }}>
          ...
          <div style={{ 
            ...
            <div style={{ marginBottom: '8px' }}>
              <strong style={{ fontSize: '16px' }}>{selectedInquiry.title}</strong>
              <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                  작성자: {selectedInquiry.userId?.username} ({selectedInquiry.userId?.email})
                </span>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                  작성일: {new Date(selectedInquiry.createdAt).toLocaleString('ko-KR')}
                </span>
              </div>
            </div>
            <div style={{ 
