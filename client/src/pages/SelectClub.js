...
export default function SelectClub(){
  ...
  useEffect(()=>{ (async()=>{ const { data } = await listClubs(); setClubs(data); })(); },[]);
  async function submit(e){ e.preventDefault(); const { data:updated } = await updateMe({ clubId }); setUser(updated); nav('/'); }
  ...
  return (
    <div className="container" style={{ maxWidth:540 }}>
      ...
      <form onSubmit={submit} className="card" style={{ display:'grid', gap:12 }}>
        ...
        <button className="btn primary" disabled={!clubId}>확인</button>
      </form>
    </div>
  );
}