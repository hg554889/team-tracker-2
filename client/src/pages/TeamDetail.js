...
export default function TeamDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data } = await getTeamJoinRequests(team._id, {
          status: "pending",
        });
  ...
  const reportsChartData = useMemo(() => {
    return reports
      ...
      .map((r) => ({
        week: new Date(r.weekOf).toLocaleDateString("ko-KR", {
          month: "short",
          day: "numeric",
        }),
        progress: r.progress,
  ...
  async function saveOverview(e) {
    ...
    window.dispatchEvent(
      new CustomEvent("toast", {
        detail: { type: "success", msg: "팀 정보가 저장되었습니다." },
      })
    );
    const { data } = await getTeam(id);
    setTeam(data || null);
  ...
  async function inviteLink() {
    ...
    window.dispatchEvent(
      new CustomEvent("toast", {
        detail: { type: "success", msg: "초대 링크가 복사되었습니다." },
      })
    );
  }
  ...
  async function changeUserRole(targetUserId, newRole) {
    try {
      ...
      const errorMessage =
        error?.response?.data?.error || "역할 변경에 실패했습니다.";
      window.dispatchEvent(
        new CustomEvent("toast", {
          detail: { type: "error", msg: errorMessage },
        })
  ...
  async function handleJoinRequest(requestId, action) {
    try {
      ...
      const errorMessage =
        error?.response?.data?.error || "처리에 실패했습니다.";
      window.dispatchEvent(
        new CustomEvent("toast", {
          detail: { type: "error", msg: errorMessage },
        })
  ...
  async function createTeamIssue() {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || "팀 이슈 생성에 실패했습니다.";
      window.dispatchEvent(
        new CustomEvent("toast", {
          detail: { type: "error", msg: errorMessage },
        })
  ...
  async function leaveTeam() {
    ...
    if (isLeader) {
      const otherLeaders = (team.members || []).filter(
        (m) => m.role === "LEADER" && String(m.user._id) !== String(user._id)
      );
      ...
      if (
        !window.confirm(
          "팀장입니다. 떠나면 다른 리더가 팀을 관리하게 됩니다. 계속하시겠습니까?"
        )
      ) {
        return;
      }
    ...
    try {
      ...
      const errorMessage =
        error?.response?.data?.message || "팀 탈퇴에 실패했습니다.";
      window.dispatchEvent(
        new CustomEvent("toast", {
          detail: { type: "error", msg: errorMessage },
        })
  ...
  if (team === null)
    return (
      <div className="container">
        <div className="card">팀을 찾을 수 없습니다.</div>
      </div>
    );
    ...
    <div className="container">
      ...
      >
          ...
          >
            ...
            <span
              style={{
                padding: "4px 8px",
                backgroundColor:
                  team.status === "ACTIVE" ? "#dcfce7" : "#fee2e2",
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          {canEdit && (
            <button className="btn" onClick={inviteLink}>
              👥 초대 링크
            </button>
          )}
          {isMember && (
      ...
      >
        {["overview", "progress", "members", "reports", "prediction", "issues"]
          .concat(isLeader && joinRequests.length > 0 ? ["requests"] : [])
          .map((t) => (
            <button
              key={t}
            ...
            <div style={{ display: "grid", gap: "16px" }}>
              ...
              >
                <div>
                  ...
                  >
                    {team.startAt
                      ? new Date(team.startAt).toLocaleDateString("ko-KR")
                      : "설정되지 않음"}
                  </p>
                </div>
                <div>
                  ...
                  >
                    {team.endAt
                      ? new Date(team.endAt).toLocaleDateString("ko-KR")
                      : "설정되지 않음"}
                  </p>
                </div>
              </div>
        ...
        >
          {reportsChartData.length === 0 ? (
            ...
            <>
              ...
              >
                {(() => {
                  const progressValues = reportsChartData.map(
                    (d) => d.progress
                  );
      ...
      {tab === "members" && (
        <Section
          ...
          right={
            canEdit ? (
              <span style={{ color: "#666", fontSize: "14px" }}>리더 전용</span>
            ) : null
          }
        >
          {canEdit && (
      ...
      {/* AI 예측 탭 */}
      {tab === "prediction" && <ProjectPrediction teamId={id} />}
                      ...
                      <td>
                        <div style={{ display: "flex", gap: "8px" }}>
                          ...
                          <button
                            className="btn"
                            onClick={() =>
                              handleJoinRequest(request._id, "reject")
                            }
                            style={{
                              backgroundColor: "#dc3545",
                              color: "white",
              ...
              {teamIssues.map((issue) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "16px",
                    backgroundColor:
                      issue.status === "resolved" ? "#f0fdf4" : "#ffffff",
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      {issue.status === "open" && (
                        <button
                          className="btn"
                          onClick={() =>
                            updateIssueStatus(issue._id, "in_progress")
                          }
                          style={{
                            fontSize: "12px",
                            padding: "4px 8px",
      ...
      {showIssueModal && (
        ...
        >
          ...
          >
            <h3
              style={{ marginTop: 0, marginBottom: "20px", fontSize: "18px" }}
            >
              새 이슈 생성
            </h3>
            ...
            >
              ...
              <button className="btn primary" onClick={createTeamIssue}>
                생성
              </button>
            </div>
          </div>
