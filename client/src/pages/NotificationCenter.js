...
export default function NotificationCenter() {
  ...
  const filteredNotifications = notifications.filter((notification) => {
    switch (filter) {
      ...
      case "team":
        return ["team_invite", "team_update"].includes(notification.type);
      case "task":
        return ["task_assigned", "report_due", "deadline"].includes(
          notification.type
        );
      default:
        return true;
  ...
  const filters = [
    ...
    },
    {
      ...
      count: notifications.filter((n) =>
        ["team_invite", "team_update"].includes(n.type)
      ).length,
    },
    {
      id: "task",
      label: "작업",
      count: notifications.filter((n) =>
        ["task_assigned", "report_due", "deadline"].includes(n.type)
      ).length,
    },
  ];
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "800px", margin: "0 auto" }}>
          ...
          {filters.map((filterItem) => (
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  filter === filterItem.id ? "#3498db" : "transparent",
      ...
      >
        <h3
          style={{ fontSize: "18px", color: "#2c3e50", marginBottom: "16px" }}
        >
          ⚙️ 알림 설정
        </h3>
