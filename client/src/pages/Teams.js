...
export default function Teams() {
  ...
  async function fetchTeams(params = {}) {
    ...
    try {
      ...
      const errorMsg =
        error?.response?.data?.message || "팀 목록을 불러오는데 실패했습니다.";
  ...
  useEffect(() => {
    fetchTeams(); /* eslint-disable-next-line */
  }, [currentClub]);
  ...
  async function onCreate(e) {
    ...
    const clubId =
      user?.role === "ADMIN" && currentClub ? currentClub : user?.clubId;
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("toast", {
          detail: { type: "success", msg: "팀이 생성되었습니다." },
        })
      );
      // 목록 새로고침
      await fetchTeams();
  ...
  async function requestToJoinTeam(teamId) {
    try {
      ...
      const errorMessage =
        error?.response?.data?.error || "가입 신청에 실패했습니다.";
      window.dispatchEvent(
        new CustomEvent("toast", {
            ...
            <tbody>
              {teams.map((t) => (
                <tr
                  key={t._id}
                  style={{
                    backgroundColor: t.userMembership?.isMember
                      ? "#f8f9ff"
                      : "transparent",
                  }}
                >
                  <td>
                    ...
                    >
                      ...
                      {t.userMembership?.isMember && (
                        <span
                          style={{
                            backgroundColor: t.userMembership.isLeader
                              ? "#28a745"
                              : "#007bff",
                            color: "white",
                            fontSize: "10px",
