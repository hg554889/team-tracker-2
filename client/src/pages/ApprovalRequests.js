...
export default function ApprovalRequests() {
  ...
  const loadPendingUsers = async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('toast', { 
        detail: { type: 'error', msg: '승인 요청 목록을 불러오는데 실패했습니다.' } 
      }));
    }
  };
  ...
  const loadRoleRequests = async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('toast', { 
        detail: { type: 'error', msg: '권한 요청 목록을 불러오는데 실패했습니다.' } 
      }));
    } finally {
      setLoading(false);
    }
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid #ddd' }}>
                    ...
                    <th style={{ padding: '12px', textAlign: 'left' }}>사용자명</th>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ marginBottom: '8px' }}>
                        <strong>{request.userId.username}</strong> ({request.userId.email})
                        <br />
                    ...
                    <div style={{ display: 'flex', gap: '8px', marginLeft: '16px' }}>
                      <button 
                        className="btn primary" 
                        style={{ fontSize: '12px', padding: '4px 8px' }}
                        onClick={() => handleRoleRequestProcess(request._id, 'approve')}
                      >
                        승인
                      </button>
                      <button 
                        className="btn secondary" 
                        style={{ fontSize: '12px', padding: '4px 8px' }}
                        onClick={() => handleRoleRequestProcess(request._id, 'reject')}
                      >
                        거절
                      </button>
                    </div>
