...
export default function ReviewManagement() {
  ...
  const handleReviewAction = async (reviewId, action, comment = '') => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('toast', { 
        detail: { type: 'success', msg: `리뷰가 ${action === 'approve' ? '승인' : '거절'}되었습니다.` } 
      }));
    } catch (error) {
  ...
  const getReviewTypeColor = (type) => {
    switch (type) {
      ...
      case 'document': return '#9b59b6';
      case 'design': return '#e74c3c';
      case 'report': return '#f39c12';
  ...
  const filteredReviews = reviews.filter(review => {
    switch (activeTab) {
      ...
      case 'completed': return ['approved', 'rejected'].includes(review.status);
      case 'my_requests': return review.requesterId === 'currentUserId'; // 실제 구현시 현재 사용자 ID 사용
      default: return true;
    }
  });

  const tabs = [
    { id: 'pending', label: '대기중', count: reviews.filter(r => r.status === 'pending').length },
    { id: 'completed', label: '완료됨', count: reviews.filter(r => ['approved', 'rejected'].includes(r.status)).length },
    { id: 'my_requests', label: '내 요청', count: reviews.filter(r => r.requesterId === 'currentUserId').length }
  ];
        ...
        {filteredReviews.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px' }}>
            ...
            <h3 style={{ fontSize: '18px', color: '#2c3e50', marginBottom: '8px' }}>
              {activeTab === 'pending' ? '대기중인 리뷰가 없습니다' : 
               activeTab === 'completed' ? '완료된 리뷰가 없습니다' : 
               '요청한 리뷰가 없습니다'}
            </h3>
              ...
              >
                <div style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{
                      ...
                      <span style={{
                        ...
                        {review.status === 'pending' ? '대기중' : 
                         review.status === 'approved' ? '승인됨' : '거절됨'}
                      </span>
                    </div>
                    ...
                    <div style={{
                      ...
                      <span>요청일: {new Date(review.createdAt).toLocaleDateString()}</span>
                      {review.dueDate && (
                        <span>마감: {new Date(review.dueDate).toLocaleDateString()}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {review.status !== 'pending' && review.reviewComment && (
                  <div style={{
                    marginTop: '16px',
                    padding: '16px',
                    background: review.status === 'approved' ? '#f0fff0' : '#fff5f5',
                    border: `1px solid ${review.status === 'approved' ? '#d4edda' : '#fed7d7'}`,
                    borderRadius: '8px'
                  }}>
                    <h4 style={{
                      fontSize: '14px',
