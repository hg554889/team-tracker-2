...
// 초대 링크 생성 (리더만)
router.post("/create", requireAuth, requireApproval, async (req, res) => {
...
router.get("/:code", async (req, res) => {
  ...
  if (!it || Date.now() > it.exp)
    return res.status(404).json({ error: "InvalidInvite" });
...
// 초대 수락 (로그인 필요)
router.post("/:code/accept", requireAuth, requireApproval, async (req, res) => {
  const it = INVITES.get(req.params.code);
  if (!it || Date.now() > it.exp)
    return res.status(404).json({ error: "InvalidInvite" });
  const team = await Team.findById(it.teamId);
